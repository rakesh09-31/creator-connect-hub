...
-- ============================================================================
-- OmniCraft Complete Stabilization & Schema Hardening Migration
-- ============================================================================

-- Step 0: Ensure Enums & Extensions
...
-- Step 1: Drop all old RLS policies across all tables to allow column type conversions
DO $$ 
DECLARE
    pol RECORD;
BEGIN
...
-- Step 2: Drop existing foreign keys that might block type changes
...
-- Step 3: Alter existing text ID columns to UUID
ALTER TABLE public.conversation_members ALTER COLUMN conversation_id TYPE UUID USING (CASE WHEN conversation_id IS NULL OR conversation_id = '' THEN NULL ELSE conversation_id::uuid END);
ALTER TABLE public.conversation_members ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.creator_requests ALTER COLUMN client_id TYPE UUID USING (CASE WHEN client_id IS NULL OR client_id = '' THEN NULL ELSE client_id::uuid END);
ALTER TABLE public.creator_requests ALTER COLUMN creator_id TYPE UUID USING (CASE WHEN creator_id IS NULL OR creator_id = '' THEN NULL ELSE creator_id::uuid END);
...
ALTER TABLE public.creator_specialties ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.follows ALTER COLUMN follower_id TYPE UUID USING (CASE WHEN follower_id IS NULL OR follower_id = '' THEN NULL ELSE follower_id::uuid END);
ALTER TABLE public.follows ALTER COLUMN following_id TYPE UUID USING (CASE WHEN following_id IS NULL OR following_id = '' THEN NULL ELSE following_id::uuid END);
...
ALTER TABLE public.job_applications ALTER COLUMN job_id TYPE UUID USING (CASE WHEN job_id IS NULL OR job_id = '' THEN NULL ELSE job_id::uuid END);
...
ALTER TABLE public.jobs ALTER COLUMN client_id TYPE UUID USING (CASE WHEN client_id IS NULL OR client_id = '' THEN NULL ELSE client_id::uuid END);
ALTER TABLE public.message_reactions ALTER COLUMN message_id TYPE UUID USING (CASE WHEN message_id IS NULL OR message_id = '' THEN NULL ELSE message_id::uuid END);
ALTER TABLE public.message_reactions ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.messages ALTER COLUMN conversation_id TYPE UUID USING (CASE WHEN conversation_id IS NULL OR conversation_id = '' THEN NULL ELSE conversation_id::uuid END);
ALTER TABLE public.messages ALTER COLUMN sender_id TYPE UUID USING (CASE WHEN sender_id IS NULL OR sender_id = '' THEN NULL ELSE sender_id::uuid END);
...
ALTER TABLE public.notifications ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.portfolios ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.post_comments ALTER COLUMN post_id TYPE UUID USING (CASE WHEN post_id IS NULL OR post_id = '' THEN NULL ELSE post_id::uuid END);
ALTER TABLE public.post_comments ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.post_likes ALTER COLUMN post_id TYPE UUID USING (CASE WHEN post_id IS NULL OR post_id = '' THEN NULL ELSE post_id::uuid END);
ALTER TABLE public.post_likes ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.post_saves ALTER COLUMN post_id TYPE UUID USING (CASE WHEN post_id IS NULL OR post_id = '' THEN NULL ELSE post_id::uuid END);
ALTER TABLE public.post_saves ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.posts ALTER COLUMN author_id TYPE UUID USING (CASE WHEN author_id IS NULL OR author_id = '' THEN NULL ELSE author_id::uuid END);
ALTER TABLE public.profile_contacts ALTER COLUMN id TYPE UUID USING (id::uuid);
ALTER TABLE public.profile_contacts ALTER COLUMN id SET DEFAULT gen_random_uuid();
...
ALTER TABLE public.squad_invites ALTER COLUMN squad_id TYPE UUID USING (CASE WHEN squad_id IS NULL OR squad_id = '' THEN NULL ELSE squad_id::uuid END);
ALTER TABLE public.squad_invites ALTER COLUMN inviter_id TYPE UUID USING (CASE WHEN inviter_id IS NULL OR inviter_id = '' THEN NULL ELSE inviter_id::uuid END);
ALTER TABLE public.squad_invites ALTER COLUMN invitee_id TYPE UUID USING (CASE WHEN invitee_id IS NULL OR invitee_id = '' THEN NULL ELSE invitee_id::uuid END);
...
ALTER TABLE public.squad_join_requests ALTER COLUMN squad_id TYPE UUID USING (CASE WHEN squad_id IS NULL OR squad_id = '' THEN NULL ELSE squad_id::uuid END);
ALTER TABLE public.squad_join_requests ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.squad_members ALTER COLUMN squad_id TYPE UUID USING (CASE WHEN squad_id IS NULL OR squad_id = '' THEN NULL ELSE squad_id::uuid END);
ALTER TABLE public.squad_members ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.squads ALTER COLUMN owner_id TYPE UUID USING (CASE WHEN owner_id IS NULL OR owner_id = '' THEN NULL ELSE owner_id::uuid END);
ALTER TABLE public.typing_status ALTER COLUMN conversation_id TYPE UUID USING (CASE WHEN conversation_id IS NULL OR conversation_id = '' THEN NULL ELSE conversation_id::uuid END);
ALTER TABLE public.typing_status ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
ALTER TABLE public.user_roles ALTER COLUMN user_id TYPE UUID USING (CASE WHEN user_id IS NULL OR user_id = '' THEN NULL ELSE user_id::uuid END);
...
-- Step 6: Foreign Keys & Relationships
DO $$ BEGIN
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'professional_roles_name_key') THEN
        ALTER TABLE public.professional_roles ADD CONSTRAINT professional_roles_name_key UNIQUE (name);
    END IF;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;
...
-- Step 7: Enable RLS & Apply Production Policies
...
-- Step 8: Stored Procedures & RPC Functions
...
-- Deduplicate skills before inserting
DELETE FROM public.skills a USING public.skills b WHERE a.id > b.id AND a.name = b.name;
