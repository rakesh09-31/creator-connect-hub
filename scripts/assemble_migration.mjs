...
console.log('--- FOUND COLUMNS TO ALTER ---');
const alterStatements = [];
for (const col of textIdColumns) {
  console.log(`${col.table_name}.${col.column_name}`);
  if (col.column_name === 'id') {
    alterStatements.push(`ALTER TABLE public.${col.table_name} ALTER COLUMN ${col.column_name} TYPE UUID USING (${col.column_name}::uuid);`);
    alterStatements.push(`ALTER TABLE public.${col.table_name} ALTER COLUMN ${col.column_name} SET DEFAULT gen_random_uuid();`);
  } else {
    alterStatements.push(`ALTER TABLE public.${col.table_name} ALTER COLUMN ${col.column_name} TYPE UUID USING (CASE WHEN ${col.column_name} IS NULL OR ${col.column_name} = '' THEN NULL ELSE ${col.column_name}::uuid END);`);
  }
}

console.log(`Generated ${alterStatements.length} alter statements.`);

// Now let's assemble the whole migration file
const fullMigration = `
-- ============================================================================
...
fs.writeFileSync('supabase/migrations/20260826000000_omnicraft_complete_stabilization.sql', fullMigration);
console.log('Saved supabase/migrations/20260826000000_omnicraft_complete_stabilization.sql');
