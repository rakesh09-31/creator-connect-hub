...
import { Home as HomeIcon, Search, PlusSquare, User as UserIcon, Briefcase, LogOut, Sun, Moon } from "lucide-react";
...
function AppShell() {
  ...
  useEffect(() => {
    ...
    const updatePresence = () => void (supabase.from("user_presence" as any) as any).upsert({ user_id: profile.id, last_seen_at: new Date().toISOString(), updated_at: new Date().toISOString() });
    updatePresence();
    const timer = window.setInterval(updatePresence, 30_000);
    window.addEventListener("focus", updatePresence);
    return () => { window.clearInterval(timer); window.removeEventListener("focus", updatePresence); };
  }, [profile?.id]);
      ...
      <header className="bg-surface/85 backdrop-blur-xl border-b border-border/60 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto px-5 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1">
            ...
            <MessageBadge active={isActive("/messages")} />
            <button
              onClick={toggle}
