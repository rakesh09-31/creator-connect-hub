    ...
    <div className="max-w-3xl mx-auto px-3 sm:px-4 py-5 space-y-4">
      ...
      {q.trim() && people.length > 0 && (
        <div className="flex gap-3 overflow-x-auto pb-1 -mx-1 px-1">
          {people.map((p) => (
            <div key={p.id} className="shrink-0 w-28 flex flex-col items-center text-center gap-1">
              <Link
                to="/user/$username"
              ...
              >
                ...
                <p className="text-[10px] text-muted-foreground capitalize truncate w-full">
                  {p.role ?? "creator"}
                </p>
              </Link>
              {p.id !== user?.id && (
                <button
                  ...
                  className="mt-1 w-full rounded-lg border border-border py-1 text-[10px] font-semibold hover:bg-muted"
                >
                  Message
                </button>
              )}
      ...
      {viewerPostId && <PostViewer startId={viewerPostId} onClose={() => setViewerPostId(null)} />}
    </div>
  );
}
