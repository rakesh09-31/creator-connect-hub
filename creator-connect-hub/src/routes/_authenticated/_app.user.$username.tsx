...
import { ProfileStoryButton } from "@/components/Stories";
...
function UserProfilePage() {
  ...
  const openDirectMessage = async () => {
    ...
    try {
      const conversationId = await openOrCreateConversation(profile.id);
    ...
    <div className="max-w-3xl mx-auto px-4 py-6">
      <div className={`rounded-3xl p-8 text-white shadow-lg ${isCreator
        ...
        <div className="flex items-start gap-5">
          <ProfileStoryButton profile={profile} className="shrink-0" />
          <div className="flex-1">
            ...
            {!isSelf && (
              <div className="mt-4 flex flex-wrap gap-2">
                <button onClick={toggleFollow} className={`px-5 py-2 rounded-xl font-semibold text-sm ${following ? "bg-white/20 text-white" : "bg-white text-gray-900"}`}>
                  {following ? "Following" : "Follow"}
                </button>
                <button onClick={() => void openDirectMessage()} className="px-5 py-2 rounded-xl font-semibold text-sm bg-white/20 text-white hover:bg-white/30">
                  Message
                </button>
              </div>
            )}
