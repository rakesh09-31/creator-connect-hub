...
function labelFor(n: Notif) {
  ...
  switch (n.type) {
    ...
    case "squad_message":
      return `${name} in ${n.data?.squad_name ?? "your squad"}: ${n.data?.preview ?? "sent a message"}`;
    case "message_reply":
      return `${name} replied to your message`;
    case "message_reaction":
      return `${name} reacted ${n.data?.emoji ?? "to your message"}`;
    case "job_application":
      return `${name} applied for your job: ${n.data?.job_title ?? "your brief"}`;
    case "job_application_accepted":
      return `Congratulations! Your application for ${n.data?.job_title ?? "the brief"} was accepted.`;
    case "job_application_rejected":
      return `Your application for ${n.data?.job_title ?? "the brief"} was not selected.`;
    case "project_assigned":
      return `You were assigned to ${n.data?.job_title ?? "a project"}.`;
    case "mention":
...
function linkFor(n: Notif): string {
  if (n.type === "job_application" && n.entity_id) return `/applications/${n.entity_id}`;
  if (n.type === "job_application_rejected") return "/jobs";
  if ((n.type === "job_application_accepted" || n.type === "project_assigned") && typeof n.data?.conversation_id === "string") return `/messages?c=${n.data.conversation_id}`;
  if (n.type === "follow" && n.actor?.username) return `/user/${n.actor.username}`;
  if (n.entity_type === "conversation" && n.entity_id) return `/messages?c=${n.entity_id}`;
  if (n.entity_type === "post" && n.entity_id) return `/home?post=${n.entity_id}`;
  ...
  const loadMore = async () => {
    ...
    if (actorIds.length) {
      const { data: profiles } = await supabase
        ...
        .select("id, username, full_name, avatar_url")
        .in("id", actorIds);
      actorMap = Object.fromEntries((profiles ?? []).map((profile) => [profile.id, profile]));
    }
    setItems((current) => {
      ...
      {loading ? (
        ...
        {hasMore && (
          <button
            ...
            className="mx-auto mt-4 block text-sm font-semibold text-brand hover:underline disabled:opacity-50"
          >
