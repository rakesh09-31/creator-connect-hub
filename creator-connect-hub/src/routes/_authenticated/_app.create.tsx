...
import { StoryComposer } from "@/components/Stories";

export const Route = createFileRoute("/_authenticated/_app/create")({
  validateSearch: (search: Record<string, unknown>) => ({ mode: search.mode === "story" ? "story" as const : undefined }),
...
function CreatePage() {
  ...
  const { mode } = useSearch({ from: "/_authenticated/_app/create" });
...
mediaUrl = await uploadMedia(file, user.id, BUCKETS.POSTS, "post");
      }
  ...
  return (
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      <div className="mb-6 flex items-center justify-between gap-3">
        ...
        <button onClick={() => navigate({ to: "/create", search: { mode: "story" } })} className="shrink-0 rounded-xl border border-brand/30 bg-brand-soft px-3 py-2 text-sm font-bold text-brand hover:bg-brand/15">+ Add story</button>
      </div>
    ...
    {mode === "story" && <StoryComposer onClose={() => navigate({ to: "/create", search: {}, replace: true })} onShared={() => navigate({ to: "/home" })} />}
    </>
  );
}
